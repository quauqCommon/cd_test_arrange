...
function findProjectList(){
	$.ajax({
		type : 'POST',
		url : "test/project/projectList",
		async : false,
		success : function(data) {
			var projectList=data["aaData"];
			$("#secProject").append("<option value='0'>无</option>");
			for(i=0;i<projectList.length;i++){
				var project=projectList[i];
				$("#secProject").append("<option value='"+project["id"]+"'>"+project["name"]+"</option>");
...
function changeProject(obj){
	$.ajax({
		type : 'POST',
		url : "test/group/groupList?parentGroupId=-1&projectId="+$(obj).val(),
		async : false,
		success : function(data) {
...
function changeGroup(obj){
	$.ajax({
		type : 'POST',
		url : "test/group/groupList?parentGroupId="+$(obj).val(),
		async : false,
		success : function(data) {
			var groupList=data["aaData"];
			$("#secGroup").empty();
			$("#secGroup").append("<option value='-1'>请选择</option>");
			for(i=0;i<groupList.length;i++){
				var group=groupList[i];
				$("#secGroup").append("<option value='"+group["id"]+"'>"+group["name"]+"</option>");
...
function showScriptList(obj){
	$.ajax({
		type : 'POST',
		url : "test/script/scriptList?groupId="+$(obj).val(),
		async : false,
		success : function(data) {
			var scriptList=data["aaData"];
			$("#secScript").empty();
			$("#secScript").append("<option value='-1'>请选择</option>");
			for(i=0;i<scriptList.length;i++){
				var script=scriptList[i];
				$("#secScript").append("<option value='"+script["id"]+"'>"+script["name"]+"_"+script["comment"]+"</option>");
...
function setCaseName(){
	$.ajax({
        type : 'POST',
        url : "test/case/caseList",
        data : {
        	"id" : $("#txtCaseId").val()
        },
        success : function(data) {
        	var caseList=data["aaData"];
        	var c=caseList[0];
        	$("#breadCaseName").text(c["name"]);
        }
    });
}
function setBaseCaseName(){
	$.ajax({
        type : 'POST',
        url : "test/baseCase/baseCaseList",
        data : {
        	"id" : $("#txtBaseCaseId").val()
        },
        success : function(data) {
        	var baseCaseList=data["aaData"];
        	var baseCase=baseCaseList[0];
        	$("#breadBaseCaseName").text(baseCase["name"]);
        }
    });
}
</script>
